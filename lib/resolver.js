...
async function clickAllServers(page, embedMap, m3u8Map, { earlyExit = false, refererUrl } = {}) {
  ...
  for (let i = 0; i < buttons.length; i++) {
    try {
      ...
      // وضع سريع: أول ما نلقى رابط شغال فعليًا نوقف — ما نحتاج نجرب
      // باقي السيرفرات (هذا اللي يخلي المسار السريع فعلاً سريع)
      if (earlyExit && m3u8Map.size > beforeCount) {
        const newest = pickBestM3u8([...m3u8Map.values()])[0]
        if (newest && (await isServerAlive(newest.url, refererUrl, 3000))) {
          log.success(`لقينا رابط شغال بعد ${i + 1} محاولة — نوقف هنا`)
...
async function searchTopCinema(browser, title, year, seasonNum, episodeNum) {
  ...
  try {
    ...
    // بعض الروابط تموت خلال ثواني من سحبها (يبدو مرتبط بجلسة/IP السحب) —
    // نتأكد فعليًا إنها شغالة قبل ما نكتبها بقاعدة البيانات، بدل ما نثق
    // برقم انتهاء مكتوب بالرابط قد يكون غير دقيق
    const refererForCheck = watchUrl || targetUrl
    const aliveChecks = await Promise.all(
      candidates.slice(0, 5).map((c) => isServerAlive(c.url, refererForCheck))
    )
    const verified = candidates.slice(0, 5).filter((_, i) => aliveChecks[i])

    if (!verified.length) {
      return {
        status: 'hls_found_but_dead',
        reason: 'تم العثور على روابط HLS لكن كلها ميتة عند التحقق الفعلي',
        matchedUrl: targetUrl, watchUrl: watchUrl || null, hlsUrls: [], elapsedMs: Date.now() - startedAt,
      }
    }
