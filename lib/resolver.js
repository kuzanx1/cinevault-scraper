...
// أسماء سيرفرات جربناها وشغالة كويس عبر بروكسي موقعنا (uqload) — نجرّبها
// أول قبل غيرها بدل الترتيب اللي يعرضه الموقع المصدر
const PREFERRED_SERVER_LABELS = ['updown', 'uqload']

async function clickAllServers(page, m3u8Map, { earlyExit = false, refererUrl } = {}) {
...
async function searchTopCinema(browser, title, year, seasonNum, episodeNum) {
  ...
  try {
    ...
    await clickAllServers(page, m3u8Map, {
      earlyExit: true,
...
// ─── مصدر ثاني: اكوام (ak.sv) ───
// بنية مختلفة تمامًا عن topcinema: ما فيه أزرار "سيرفرات" لمواقع
// تضمين خارجية — عندهم مشغّل فيديو خاص فيهم، والفيديو نفسه رابط
// مباشر (mp4) من CDN خاص فيهم (downet.net). أثبتنا فعليًا إنه مستقل
// عن كل مزودي topcinema المحجوبين (cdn-video.xyz, mxcontent.net).
...
async function refreshTitle(browser, titleId, episodeId = null, reporter = null) {
  ...
  let result = await searchTopCinema(browser, title.original_title, title.year, seasonNum, episodeNum)
  ...
  // topcinema ما لقى شي — نجرّب اكوام كمصدر ثاني (أفلام بس حاليًا).
  // مصدر مستقل تمامًا (CDN خاص فيهم downet.net)، مو نفس مزودي topcinema
  if (result.status !== 'success' && !episodeId) {
    const akwamResult = await searchAkwam(browser, title.original_title, title.year, seasonNum, episodeNum)
    reporter?.appendAttempt({
      titleId, episodeId, title: title.original_title, year: title.year,
      status: `akwam_${akwamResult.status}`, reason: akwamResult.reason, matchedUrl: akwamResult.matchedUrl,
      hlsCount: akwamResult.hlsUrls.length, elapsedMs: akwamResult.elapsedMs,
    })
    if (akwamResult.status === 'success') result = akwamResult
  }
