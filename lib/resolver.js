...
// ─── (2) البحث بمواقع "اكوام العائلة" (اكوام + افلام دوت كوم) ───
// شلنا topcinema بالكامل — كل مزوديه (cdn-video.xyz, streamwish,
// videotube..) طلعوا محجوبين فعليًا من بنيتنا، أثبتناها بفك تشفير
// وتجربة مباشرة. اكتشفنا موقعين مختلفين (كتالوج مختلف لكل وحد) لكنهم
// يشتغلون على نفس CDN المباشر غير المحجوب (downet.net) — نفس مشغّل
// native، رابط الفيديو مباشر mp4 بدل embed خارجي. أفلام بس حاليًا.
const DOWNET_MEDIA_HOSTS = ['downet.net']

const DOWNET_SITES = [
  { name: 'اكوام', searchUrl: (t) => `https://ak.sv/search?q=${encodeURIComponent(t)}` },
  { name: 'افلام دوت كوم', searchUrl: (t) => `https://aflaam.com/search?q=${encodeURIComponent(t)}` },
]

async function searchDownetSite(browser, site, title, year, seasonNum, episodeNum) {
  const startedAt = Date.now()

  if (seasonNum && episodeNum) {
    return {
      status: 'unsupported', reason: `${site.name} يدعم الأفلام بس حاليًا`,
      matchedUrl: null, watchUrl: null, hlsUrls: [], elapsedMs: Date.now() - startedAt,
    }
  }

  // دمج السنة بنص البحث يكسر نتائج هالمواقع (جربناها فعليًا على اكوام،
  // ترجع صفر نتائج) — نبحث بالعنوان الخام بس، ونستخدم السنة كمرشّح
  // تحقق بعدين
  const searchUrl = site.searchUrl(title)

  log.info(`[${site.name}] يبحث عن: "${title}"`)
  ...
  page.on('request', (req) => {
    ...
    // المباشر اللي نبيه نلتقطه (المشغّل native، مو iframe)
  ...
  page.on('response', (response) => {
    try {
      ...
      if (
        DOWNET_MEDIA_HOSTS.some((h) => url.includes(h)) &&
  ...
  try {
    ...
    // هالمواقع ترجّع نتائج بعناوين متشابهة (Godfather, Godfather Part
    // II, Godfather of Northeast China...) — تطابق كلمات فضفاض زي
    // topcinema يلقط أول واحد غلط. نطابق النص بالكامل (بعد تطبيع) +
    // نتحقق من السنة المعروضة جنب كل نتيجة كمرشّح إضافي. تدرّج من
    // الأدق (تطابق تام) للأوسع (احتواء) — بعض المواقع (افلام دوت كوم)
    // تحط تقييم/جودة قبل العنوان بنفس الرابط فما يطابق تام أبدًا
    const resultUrl = await page.evaluate((t, y) => {
      ...
      for (const a of document.querySelectorAll('a[href*="/movie/"]')) {
        ...
        if (!text) continue
        let yearText = ''
        let parent = a.closest('.entry-box, .col-lg-2, .grid-item, li, .movie-card, .card') || a.parentElement
      ...
      for (const c of candidates) if (c.text.includes(targetNorm) && (!y || c.yearText === String(y))) return c.href
      return null
    }, title, year)
    ...
    if (!resultUrl) {
      await context.close()
      return {
        status: 'source_not_found', reason: `لم يتم العثور على نتيجة مطابقة بـ${site.name}`,
        matchedUrl: null, watchUrl: null, hlsUrls: [], elapsedMs: Date.now() - startedAt,
      }
    }
...
// يجرّب كل مواقع downet بالترتيب، يوقف أول نجاح
async function searchAllSources(browser, title, year, seasonNum, episodeNum, reporter, ctx) {
  let last = null
  for (const site of DOWNET_SITES) {
    const result = await searchDownetSite(browser, site, title, year, seasonNum, episodeNum)
    reporter?.appendAttempt({
      ...ctx,
      status: `${site.name}_${result.status}`, reason: result.reason, matchedUrl: result.matchedUrl,
...
async function refreshTitle(browser, titleId, episodeId = null, reporter = null) {
  ...
  const result = await searchAllSources(
    browser, title.original_title, title.year, seasonNum, episodeNum,
    reporter, { titleId, episodeId, title: title.original_title, year: title.year }
  )
